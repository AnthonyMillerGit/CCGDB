    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {attrs.cost && attrs.cost !== '-' && (
          <Chip label={`Cost ${attrs.cost}`} bg="#1a1a24" border="#42424e" text="#ffe082" />
        )}

        {/* Trait */}
        {attrs.trait && attrs.trait !== '-' && (
          <Chip label={attrs.trait} bg="#22222a" border="#32323c" text="#8e8e9e" />
        )}
      ...
      {attrs.link && attrs.link !== '-' && (
        <div className="rounded-lg border mb-3 px-3 py-2"
          ...
          <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#8e8e9e' }}>Link </span>
          <span className="text-xs" style={{ color: '#EDF2F6' }}>{attrs.link}</span>
        </div>
      )}
