...
const SUBTYPE_STYLES = {
  'Basic':         { bg: '#2a2a34', border: '#42424e',  text: '#aaa' },
  ...
  'Trainer':       { bg: '#2a2a34', border: '#42424e',  text: '#aaa' },
  'Special':       { bg: '#251520', border: '#880e4f',  text: '#f48fb1' },
  'Basic Energy':  { bg: '#2a2a34', border: '#42424e',  text: '#8e8e9e' },
}
const DEFAULT_SUBTYPE = { bg: '#2a2a34', border: '#42424e', text: '#aaa' }
  ...
  return (
    <div className="mb-3 rounded-lg overflow-hidden border" style={{ borderColor: '#c62828' }}>
      <div className="flex items-center gap-2 px-3 py-2" style={{ backgroundColor: '#3a1212' }}>
        ...
        <span className="font-bold text-sm" style={{ color: '#EDF2F6' }}>{ability.name}</span>
      </div>
      {ability.text && (
...
function AttackBlock({ attack }) {
  return (
    <div className="mb-2 rounded-lg border px-3 py-2.5" style={{ backgroundColor: '#2a2a34', borderColor: '#42424e' }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 flex-wrap">
          <EnergyCost cost={attack.cost} />
          <span className="font-bold text-sm" style={{ color: '#EDF2F6' }}>{attack.name}</span>
        </div>
        {attack.damage && (
...
function CombatStats({ attrs }) {
  ...
  return (
    <div className="flex flex-wrap gap-5 px-4 py-3 rounded-lg mt-3"
      style={{ backgroundColor: '#22222a', border: '1px solid #32323c' }}>
      {hasWeakness && (
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-wide" style={{ color: '#8e8e9e' }}>Weakness</span>
      ...
      {hasResistance && (
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-wide" style={{ color: '#8e8e9e' }}>Resist</span>
      ...
      {hasRetreat && (
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-wide" style={{ color: '#8e8e9e' }}>Retreat</span>
...
function LegalityRow({ legalities }) {
  ...
  return (
    <div className="mt-5 mb-2">
      <p className="text-xs uppercase tracking-widest mb-2" style={{ color: '#8e8e9e' }}>Legality</p>
    ...
    <div>
      ...
      {(stageType || trainerType || otherSubtypes.length > 0) && (
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          {attrs.evolvesFrom && (
            <span className="text-xs px-2 py-0.5 rounded" style={{ color: '#8e8e9e', border: '1px solid #32323c' }}>
              Evolves from <span style={{ color: '#EDF2F6' }}>{attrs.evolvesFrom}</span>
            </span>
          )}
          {otherSubtypes.map(s => {
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mb-5 pb-4"
          ...
          {attrs.level && (
            <div className="flex items-baseline gap-1">
              <span className="text-xs uppercase tracking-wide" style={{ color: '#8e8e9e' }}>Lv.</span>
              <span className="text-sm font-semibold" style={{ color: '#EDF2F6' }}>{attrs.level}</span>
            </div>
          )}
