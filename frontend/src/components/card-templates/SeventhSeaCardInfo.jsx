...
const FACTION_STYLES = {
  ...
  'Unaligned':         { text: '#8e8e9e', bg: '#2a2a34', border: '#42424e' },
  'Vendel':            { text: '#4caf50', bg: '#1a3a1a', border: '#3aa040' },
  'Vestenmannavnjar':  { text: '#e67e22', bg: '#3a2a1a', border: '#b06010' },
}
const DEFAULT_FACTION = { text: '#8e8e9e', bg: '#2a2a34', border: '#42424e' }
...
function StatBox({ label, value, color, wide }) {
  ...
  return (
    <div className="flex flex-col items-center px-3 py-2 rounded-lg border"
      style={{ backgroundColor: '#2a2a34', borderColor: '#42424e', minWidth: wide ? '80px' : '52px' }}>
      <span className="text-xs uppercase tracking-wide mb-0.5" style={{ color: '#8e8e9e' }}>{label}</span>
      <span className="text-lg font-bold leading-tight" style={{ color: color || '#EDF2F6' }}>
        {value === 0 ? '0' : (value ?? '—')}
      </span>
    </div>
  )
...
function SkillRow({ attrs }) {
  ...
  return (
    <div className="mb-5">
      <p className="text-xs uppercase tracking-widest mb-2" style={{ color: '#8e8e9e' }}>Skills</p>
        ...
        {SKILLS.map(({ key, label, color }) => {
          ...
          return (
            <div key={key} className="flex flex-col items-center px-3 py-2 rounded-lg border"
              style={{ backgroundColor: '#2a2a34', borderColor: color + '55', minWidth: '64px' }}>
              <span className="text-xs font-semibold mb-0.5" style={{ color }}>{label}</span>
              <span className="text-xl font-bold" style={{ color: '#EDF2F6' }}>
...
function CombatRow({ attack, parry, boarding }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-4 mb-4 px-4 py-2 rounded-lg"
      style={{ backgroundColor: '#26262e', border: '1px solid #38384a' }}>
      <span className="text-xs uppercase tracking-widest font-semibold" style={{ color: '#8e8e9e' }}>Combat</span>
      {boarding && (
        <div className="flex items-center gap-1.5">
          <span className="text-xs" style={{ color: '#8e8e9e' }}>Boarding:</span>
          <span className="text-xs font-bold" style={{ color: '#EDF2F6' }}>{boarding}</span>
        </div>
      )}
      {atkName && (
        <div className="flex items-center gap-1.5">
          <span className="text-xs" style={{ color: '#8e8e9e' }}>Attack:</span>
          <span className="text-xs font-bold" style={{ color: '#e74c3c' }}>{atkName}</span>
        </div>
      )}
      {parryArr.length > 0 && (
        <div className="flex items-center gap-1.5">
          <span className="text-xs" style={{ color: '#8e8e9e' }}>Parry:</span>
...
function ErrataBox({ text }) {
  ...
  return (
    <div className="rounded-xl p-4 mb-4 border"
      ...
      <p className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: '#6A7EFC' }}>Errata</p>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        {startPort && (
          <div className="flex flex-col justify-center px-4 py-2 rounded-lg border"
            ...
            <span className="text-xs uppercase tracking-wide mb-0.5" style={{ color: '#8e8e9e' }}>Home Port</span>
            <span className="text-sm font-semibold" style={{ color: '#EDF2F6' }}>{startPort}</span>
          </div>
        )}
        {alignment && (
          <div className="flex flex-col justify-center px-4 py-2 rounded-lg border"
            ...
            <span className="text-xs uppercase tracking-wide mb-0.5" style={{ color: '#8e8e9e' }}>Alignment</span>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {distance && (
          <div className="flex flex-col items-center px-4 py-2 rounded-lg border"
            ...
            <span className="text-xs uppercase tracking-wide mb-0.5" style={{ color: '#8e8e9e' }}>Distance</span>
        ...
        {attrs.cost_raw && (
          <div className="flex flex-col justify-center px-4 py-2 rounded-lg border"
            style={{ backgroundColor: '#2a2a34', borderColor: '#42424e' }}>
            <span className="text-xs uppercase tracking-wide mb-0.5" style={{ color: '#8e8e9e' }}>Completion Cost</span>
            <span className="text-sm font-semibold" style={{ color: '#EDF2F6' }}>{attrs.cost_raw}</span>
          </div>
        )}
        {attachType && (
          <div className="flex flex-col items-center px-4 py-2 rounded-lg border"
            style={{ backgroundColor: '#1a2a1a', borderColor: '#3a803055' }}>
            <span className="text-xs uppercase tracking-wide mb-0.5" style={{ color: '#8e8e9e' }}>Becomes</span>
    ...
    <>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        {(attrs.cost_raw || attrs.cost) && (
          <StatBox label="Cost" value={attrs.cost_raw || attrs.cost} color="#EDF2F6" wide />
        )}
        {attrs.cancel != null && (
          <div className="flex flex-col items-center px-4 py-2 rounded-lg border"
            style={{ backgroundColor: '#2a2a34', borderColor: '#42424e', minWidth: '80px' }}>
            <span className="text-xs uppercase tracking-wide mb-0.5" style={{ color: '#8e8e9e' }}>Cancel</span>
...
function AttachmentLayout({ card, attrs, fStyle }) {
  ...
  return (
    <>
      ...
      {(attrs.cost_raw || attrs.cost) && (
        <div className="mb-4">
          <span className="text-xs uppercase tracking-wide mr-2" style={{ color: '#8e8e9e' }}>Cost:</span>
          <span className="text-sm font-semibold" style={{ color: '#EDF2F6' }}>
            {attrs.cost_raw || attrs.cost}
          </span>
        </div>
      )}
