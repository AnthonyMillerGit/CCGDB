...
def get_collection(current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def add_to_collection(body: CollectionAddRequest, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
...
def update_collection_quantity(printing_id: int, body: CollectionUpdateRequest, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def remove_from_collection(printing_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
...
def get_collection_for_set(set_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def get_collection_item(printing_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def list_decks(current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
def create_deck(body: CreateDeckRequest, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
...
def get_deck(deck_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
...
def update_deck(deck_id: int, body: UpdateDeckRequest, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
...
def delete_deck(deck_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            _get_deck_or_403(cur, deck_id, current_user["id"])
...
def add_card_to_deck(deck_id: int, body: DeckCardRequest, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            deck = _get_deck_or_403(cur, deck_id, current_user["id"])
...
def update_deck_card(deck_id: int, card_id: int, body: DeckCardUpdateRequest, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            _get_deck_or_403(cur, deck_id, current_user["id"])
...
def remove_card_from_deck(deck_id: int, card_id: int, current_user=Depends(get_current_user)):
    conn = get_db()
    try:
        with conn.cursor() as cur:
            _get_deck_or_403(cur, deck_id, current_user["id"])
